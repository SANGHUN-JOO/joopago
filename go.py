...
class Position():
    def __init__(self, board=None, n=0, komi=6, caps=(0, 0),
    ...
    def all_legal_moves(self):
        ...
        # I don't want to use a human knowledge but I have just one gpu card resource for training... so I inserted these some of below codes to help reducing calculation.
        if self.n < 6:
            edge = np.zeros([N, N], dtype=np.int8)
            edge[1:-1, 1:-1] = 1
            legal_moves = legal_moves*edge
    ...
    def pass_move(self, mutate=False):
        ...
        pos.board_deltas = np.concatenate((
            ...
            pos.board_deltas[:2])) # pos.board_deltas[:6])) for feature size 16
        pos.to_play *= -1
        pos.ko = None
        return pos
    ...
    def play_move(self, c, color=None, mutate=False):
        ...
        if not self.is_move_legal(c):
            raise IllegalMove("{} move at {} is illegal: \n{}".format(
                "Black" if self.to_play == BLACK else "White", c, self))
    ...
    def is_game_over(self):
        return ((len(self.recent) >= 2 and
                ...
                self.recent[-2].move is None) or (self.n > 180)) #orignal self.n > 722
